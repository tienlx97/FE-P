...
export function SupplierFormDialog({ isOpen, onOpenChange, supplier = null, onSuccess }) {
  const form = useSupplierForm({ supplier, onSuccess: (saved) => { onOpenChange(false); onSuccess?.(saved); } });
  /** @param {boolean} next */
  function handleOpenChange(next) { if (!next) form.reset(); onOpenChange(next); }
  return (
    <FormDialog
      isOpen={isOpen}
