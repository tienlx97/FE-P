...
## 2026-09-16 — AdvanceTable: real PowerSearch, verified SegmentedControl/Pagination already real

- **Context:** follow-up to `redesign-theme-stone` (merged to `main` and
  pushed earlier today). User asked for three things on the shared list
  table: (1) use `SegmentedControl` for the contract status quick-filter
  tabs, (2) confirm/use Astryx's real `Pagination`, (3) use `PowerSearch`
  with the table.
- **Findings for (1) and (2): already true, no code changed.**
  `contracts-list.jsx`'s status tabs already render via real
  `SegmentedControl`/`SegmentedControlItem` (`@astryxdesign/core/
  SegmentedControl`), and `advance-table-pagination.jsx` already wires
  Astryx's real `Pagination` (`@astryxdesign/core/Pagination`, `variant=
  "pages"`). Verified live (logged-in session, `/logistics/contracts`):
  clicking a status tab correctly gets the white-pill + bold "selected"
  treatment (confirmed via `getComputedStyle`, matching the
  already-working "Mặc định/Tài chính" view-preset SegmentedControl next
  to it) — the tabs looked like plain unstyled text at first glance only
  because no status was selected yet (`aria-checked="false"` on all four).
- **(3) was a real gap — fixed.** `advance-table.jsx` (the shared list
  table every feature list page renders through) already used
  `usePowerSearchConfig`'s `config`/`applyFilters` for its filter *engine*,
  but rendered a plain `InputGroup`+`TextInput` shim for the actual search
  *UI* instead of Astryx's real `<PowerSearch>` token bar — exactly the gap
  Astryx's own `PowerSearchSearchWithTable` block template (`astryx
  template PowerSearchSearchWithTable`) demonstrates fixing. Swapped it in:
  - `searchConfig`/`searchFilters`/`handleSearchFiltersChange` were already
    present (built for this, unused by the old TextInput path) — wired
    `<PowerSearch config={searchConfig} filters={searchFilters}
    onChange={handleSearchFiltersChange} resultCount={pagination ?
    pagination.totalCount : filteredData.length} .../>` in their place, with
    the existing "Bộ lọc nâng cao" funnel `IconButton` moved into
    PowerSearch's `endContent` slot (so `AdvanceTableSearchDialog`, the
    static advanced-filter dialog for fields PowerSearch's config doesn't
    cover, is still reachable exactly as before).
  - `handleSearchFiltersChange` grew the debounced `onContentSearchChange`
    forwarding that used to live only in the deleted `handleQuickSearchChange`
    (so a caller's server-side search, e.g. contracts-list.jsx's contract-
    number search, still fires on typing) — now derives the forwarded value
    from whichever filter has `field === contentSearchFieldKey` inside the
    full filters array PowerSearch hands back, instead of a dedicated
    single-field callback. Minor behavior expansion, not a regression: this
    now also fires if that field is edited via the advanced dialog, which
    the old TextInput-only path never did.
  - Deleted now-dead code: `quickSearchValue`, `handleQuickSearchChange`,
    the `InputGroup`/`TextInput` imports, `styles.searchInputGroup`/
    `styles.searchInput`.
  - Verified live on two different callers (`/logistics/contracts` and
    `/logistics/shipments`, different `searchFieldDefs` each): PowerSearch's
    field menu lists every searchable field with correct Vietnamese labels,
    adding/removing a token filters the table and updates the inline result
    count, the funnel button still opens the advanced dialog, and the
    status-tab quick filter + advanced dialog filter + PowerSearch token all
    combine correctly (`Đang áp dụng N điều kiện lọc` tracked all three).
    No console errors observed.
- **Verification:** `./harness/verify.sh` full green —
  `harness/runs/20260916-234359-934/` (theme:build, lint, typecheck,
  structure, harness tests, unit tests, build, quality thresholds).
- Not committed yet — this is a shared-component change (cascades to every
  `AdvanceTable` caller: contracts, shipments, commissions, customers,
  suppliers, users, backups, BOQ) but scoped to one file; user hasn't asked
  for a commit this round yet, following the same wait-for-explicit-ask
  pattern as the rest of this session.

## 2026-09-17 — PowerSearch style/behavior audit: found + fixed a real filter bug

- **Context:** user asked to check/fix styling after the PowerSearch swap
  above. Visual audit (alignment, border/radius/height vs. other inputs,
  focus state, token-chip rendering, entity/enum dropdown) found no visual
  regressions — `astryx-tokenizer`'s border (`1px solid rgb(131,131,138)`),
  `border-radius` (`8px`), and height (~31-32px) match a reference
  `TextInput` on `/design-system` exactly; alignment with the adjacent
  "Mặc định/Tài chính"/"Tuỳ chọn hiển thị" controls is clean.
- **Found a real bug while stress-testing the newly-reachable field menu**
  (not a style issue, a filtering one): picking `Khách hàng` (an enum-type
  field, only reachable via the advanced-filter dialog before — that
  dialog's own field list also included it, so this bug predates
  PowerSearch, it just went unexercised) and selecting an exact customer
  name, e.g. "CG FRAME CO., LTD", returned **zero rows** even though
  multiple visible rows had that exact customer. Root cause: in
  `advance-table.jsx`'s `applyFiltersDiacriticInsensitive` (added
  2026-09-14 for diacritic-insensitive text search), the row clone
  normalized (lowercased + diacritics stripped) **every string field
  unconditionally**, while only *string-type filter values* got the same
  normalization — an enum filter's value is compared exact-case on
  purpose, so it was being checked against a lowercased row field it could
  never match. Fixed by only normalizing a row field when a *string*-type
  filter actually targets that field (mirrors the filter-side conditional
  exactly) — `git log` confirms this exact asymmetry existed before
  today's session, PowerSearch's field menu just made the affected field
  type (enum) directly reachable for the first time instead of buried in
  a secondary dialog.
- Verified fixed live (`/logistics/contracts`, real logged-in session):
  `Khách hàng: bằng CG FRAME CO., LTD` now correctly returns all 5 matching
  rows. Re-ran `./harness/verify.sh` — full green,
  `harness/runs/20260917-000120-618/`.
- **Known limitation, not fixed (flagged to user, needs a product call, not
  a style fix):** PowerSearch's `resultCount` badge can read as "34 kết
  quả" while a client-only filter (like the `Khách hàng` case above)
  narrows the visible table to far fewer rows. Root cause: `resultCount`
  uses `pagination.totalCount` (the server's true across-all-pages total)
  whenever a caller is server-paginated, which is correct for
  server-routed filters but not for `searchFieldDefs` fields that were
  never added to `filterFieldDefs` (so they only ever filter the
  already-fetched page client-side) — a pre-existing client/server search
  split this change didn't create and can't safely resolve unilaterally
