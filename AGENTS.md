...
| Run dev alongside a production instance | `.env.development.example` (API_BASE_URL → BE dev stack :8081) + `pnpm dev -p 3001` (not `-- -p`: pnpm passes the `--` through and Next reads `-p` as the project dir) — never `pnpm dev` bare, it collides with prod's port 3000 |
